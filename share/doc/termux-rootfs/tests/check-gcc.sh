...
##
##  Check if GCC (gcc/g++) is working by compiling and running simple programs
##
...
EOF
    ...
    if [ -f "${PROG_PATH}" ]; then
        if gcc -O3 "${PROG_PATH}" -o "${PROG_PATH_BIN}" -lm >/dev/null 2>&1; then
...
EOF
    ...
    if [ -f "${PROG_PATH}" ]; then
        if g++ -O3 "${PROG_PATH}" -o "${PROG_PATH_BIN}" >/dev/null 2>&1; then
            echo "[*] Successfully compiled. Running tests..."
