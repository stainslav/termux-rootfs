...
sub unpack_logo {
	...
	if (-e "/data/data/com.termux/files/usr/share/mtktools/logo_res.txt") {
		open (LOGO_RESFILE, "/data/data/com.termux/files/usr/share/mtktools/logo_res.txt")
			or die_msg("couldn't open file '/data/data/com.termux/files/usr/share/mtktools/logo_res.txt'!");
		while (<LOGO_RESFILE>) {
			if ($_ =~ /^\[(\d+),(\d+),(.*)\]$/) {
	...
	my $GraphicsMagick_installed = system ("command -v gm >/dev/null 2>&1") ? 0 : 1;
...
sub rgb565_to_png {
	...
	system ("gm convert -depth 8 -size $img_resolution rgb:$filename.raw $filename.png");
