...
our $config_unzip = '/data/data/com.termux/files/usr/bin/unzip';	# Windows path like 'C:/path/to/unzip.exe'
...
if ($ENV{OS} =~ /^Windows/ && !(exists $ENV{OSTYPE} || exists $ENV{HOME})) {
    ...
    $systemConfigDir = "/data/data/com.termux/files/usr/etc";

    $config_tempDir = "/data/data/com.termux/files/usr/tmp";
}
